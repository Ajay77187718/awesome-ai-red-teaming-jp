...
LLMを組み込んだアプリケーションは、プロンプトインジェクションやジェイルブレイクなどの攻撃に晒されます。こうした脅威に対して、攻撃者の視点からシステムの安全性を検証する手法が **AI Red Teaming** です。2026年8月にはEU AI Actで高リスクAIへのレッドチーミング文書化が義務化されるなど、実務上の重要性が急速に高まっています。
...
|------------------------|--------------|
| LLMアプリを開発・運用している | [攻撃手法](#攻撃手法) → [ツール](#ツール) → [防御手法](#防御手法) |
| 規制対応・リスク管理を担当している | [規制・フレームワーク](#規制フレームワーク) → [ツール](#ツール) |
| AI Safetyをこれから学びたい | [学習リソース](#学習リソース) → [攻撃手法](#攻撃手法) |
| AIエージェントとしてリサーチしている | [MCP / エージェントセキュリティ](#mcp--エージェントセキュリティ) → [論文](#論文) |
...
AI Red Teaming の主要なオープンソースツール。スター数は2026年4月時点。
...
- [Japan-AISI/aisev](https://github.com/Japan-AISI/aisev) - AIセーフティ研究所（AISI）が開発したAIセーフティ評価環境。10の評価観点に基づく定量・定性評価、自動レッドチーミング機能を搭載。Docker必要。日英両言語対応（2025年9月公開、2026年2月最終更新）
...
- **間接インジェクション**: 外部データソース（Webページ、ドキュメント等）に攻撃プロンプトを埋め込み、RAGシステム経由でモデルに注入 — 📄 [Greshake et al., 2023](https://arxiv.org/abs/2302.12173)
...
- **多段階攻撃 (Crescendo)**: 段階的に制約を緩和させる対話ベースの攻撃 — 📄 [Microsoft Research](https://arxiv.org/abs/2404.01833)

### 多言語攻撃

- **低リソース言語攻撃**: 日本語を含む非英語言語でのプロンプトにより、英語で訓練された安全性ガードレールをバイパス。低リソース言語では有害コンテンツに遭遇する確率が約3倍 — 📄 [Deng et al., 2024](https://arxiv.org/abs/2310.06474)
...
- **入力フィルタリング**: プロンプトの前処理による悪意ある入力の検出・ブロック — 📄 [Llama Guard](https://arxiv.org/abs/2312.06674)
- **出力フィルタリング**: モデル応答の後処理による有害コンテンツの検出・除去 — 📄 [NeMo Guardrails](https://arxiv.org/abs/2310.10501)
- **多段階防御**: 入力ガード → モデル → 出力ガード の多層防御アーキテクチャ
- **Constitutional AI**: AIフィードバックに基づく安全性アラインメント — 📄 [Anthropic, 2022](https://arxiv.org/abs/2212.08073)
